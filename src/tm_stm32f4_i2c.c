...
void TM_I2C_Init(I2C_TypeDef* I2Cx, uint32_t clockSpeed) {
	...
	uint32_t pinpos;
	uint16_t GPIO_Pin = GPIO_PIN_6 | GPIO_PIN_9;

	//Enable GPIOx clock
	TM_GPIO_INT_EnableClock(GPIOB);

	GPIOB->AFR[6 >> 0x03] =
			(GPIOB->AFR[6 >> 0x03] & ~(0x0F << (4 * (6 & 0x07))))
					| (GPIO_AF_I2C1 << (4 * (6 & 0x07)));

	GPIOB->AFR[9 >> 0x03] =
				(GPIOB->AFR[9 >> 0x03] & ~(0x0F << (4 * (9 & 0x07))))
						| (GPIO_AF_I2C1 << (4 * (9 & 0x07)));

	//Do initialization
	TM_GPIO_INT_Init(GPIOB, GPIO_Pin, TM_GPIO_Mode_AF, TM_GPIO_OType_OD, TM_GPIO_PuPd_UP, TM_GPIO_Speed_Medium);
